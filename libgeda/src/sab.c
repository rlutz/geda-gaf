...
char* action_text[]={"discard","bypass","exec"};

void* ParseBypassArgs(char* args)
{
    ...
    for(GSList* cur=arg_list;cur!=NULL;cur=cur->next)
    {
     ...
     char* as;
         ...
         if((as=strstr(pin_list,"as"))!=NULL)
         {
              ...
              while(as!=pin_list && isspace(*(as-1))) as--;
...
gchar* ConstructBypassArg(GSList* arg_list)
{
 gchar* buf1;
 gchar* buf2;
 gchar* buf3;
 GSList* cur_seg=arg_list;
    ...
    while(cur_seg!=NULL)
    {
     ...
     GSList* cur_pin=pset->pins;
        ...
        while(cur_pin!=NULL)
        {
            buf3=g_strdup_printf("%s%s%d",buf2,(strlen(buf2)>0?",":""),GPOINTER_TO_UINT(cur_pin->data));
            g_free(buf2);
            buf2=buf3;
            cur_pin->data=NULL;
...
sab_action_set *SabGetSet(OBJECT* object)
{
 ...
 char attr_buf[PARAM_BUFFER_LENGTH];
    ...
    while((attr=o_attrib_find_attrib_by_name(object->attribs,"sab-param",attr_index++))!=NULL)
    {
         ...
         sab_action* act=(sab_action*)action->data;
            if(act!=NULL)
            {
             ...
             gchar* text=NULL;
                ...
                if(act->order>-1)
                {
                    order=g_strdup_printf(":#%d:%s",act->order,action_text[act->action]);
                }
                else
                {
                ...
                text=g_strconcat(label,order,((param!=NULL && strlen(param)>0)?":":""),param,NULL);
                g_free(label);
                g_free(order);
                g_free(param);
