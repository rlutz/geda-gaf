...
/* o_arc_basic.c */
OBJECT *o_arc_new(TOPLEVEL *toplevel, int color, int x, int y, int radius, int start_angle, int sweep_angle);
...
void o_glist_translate_world(const GList *list, int dx, int dy);
...
void o_path_translate_world(OBJECT *object, int x, int y);
...
UNDO *s_undo_add(UNDO *head, int type, char *filename, GList *object_list, int x, int y, double scale, int page_control, int up, int CHANGED, const char *desc);
