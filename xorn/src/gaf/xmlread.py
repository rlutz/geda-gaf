    ...
    def start_element(self, name, attributes):
        ...
        if name == 'symbol':
            ...
            try:
                ...
                name = None
            if is_embedded:
                symbol = gaf.ref.Symbol(name, None, True)
            else:
                symbol = self.c.load_symbol(name, read_symbol)
                if symbol is None:
                    symbol = gaf.ref.Symbol(name, None, False)
                    is_embedded = True
        ...
        if name == 'pixmap':
            ...
            try:
                ...
                name = None
            if is_embedded:
                pixmap = gaf.ref.Pixmap(name, None, True)
            else:
                pixmap = self.c.load_pixmap(name, read_pixmap)
                if pixmap is None:
                    pixmap = gaf.ref.Pixmap(name, None, False)
                    is_embedded = True
