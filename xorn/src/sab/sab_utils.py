...
def exec_extern(nets, component, param):
    ...
    if (script is not None
        ...
        sys.stderr.write("WARNING: Script %s missing 'sab_process' function."
                           % (script.__name__))
    elif not isinstance(script.sab_process, types.FunctionType):
        sys.stderr.write("WARNING: %s.sab_process is not a function."
                           % (script.__name__))
    else:
        sys.stderr.write('WARNING: %s.sab_process must take three parameters.'
                           % (script.__name__))
...
def bypass(nets, component, shorts):
    ...
    for short in short_list:
        ...
        if not parts[0].strip().replace(',', '').isdigit():
            sys.stderr.write("WARNING: Only digits and commas allowed in "
        ...
        if len(pins) < 2:
            sys.stderr.write("WARNING: Two or more pins needed in shorting "
        ...
        for pin in pins[1:]:
            if pin not in component.cpins_by_number:
                sys.stderr.write('WARNING: Component %s does not have a pin '
                                 '%s. Ignoring.\n'
